...
class EasyBackupService
{
  ...
  public function __construct(
    ...
    $this->kimaiRootPath =
      \dirname(\dirname($dataDirectory)) . DIRECTORY_SEPARATOR;
  ...
  private function log(string $logLevel, string $message): void
  {
    ...
    try {
      ...
      $this->filesystem->appendToFile(
        $logFile,
        "[$dateTime] $logLevel: $message" . PHP_EOL,
      );
    } catch (\Exception $e) {
  ...
  public function getBackupDirectory(): string
  {
    return str_replace(
      ["/", "\\"],
      DIRECTORY_SEPARATOR,
      $this->kimaiRootPath . $this->configuration->getBackupDir(),
    );
  }
  ...
  public function createBackup()
  {
    ...
    $this->log(
      self::LOG_INFO_PREFIX,
      "--- S T A R T   C R E A T I N G   B A C K U P ---",
    );
    ...
    $this->log(
      self::LOG_INFO_PREFIX,
      "Creating backup dir '$pluginBackupDir'.",
    );
    ...
    $this->log(
      self::LOG_INFO_PREFIX,
      "Creating manifest file '$manifestFile'.",
    );
    ...
    try {
      ...
      if ($returnValue !== 0) {
        $this->log(
          self::LOG_WARN_PREFIX,
          "Command failed with exit code: $returnValue",
        );
      }
    } catch (\Exception $ex) {
    ...
    $this->filesystem->appendToFile(
      $manifestFile,
      \strval(json_encode($manifest, JSON_PRETTY_PRINT)),
    );
    ...
    foreach ($arrayOfPathsToBackup as $filename) {
      $sourceFile = str_replace(
        ["/", "\\"],
        DIRECTORY_SEPARATOR,
        $this->kimaiRootPath . $filename,
      );
      $targetFile = str_replace(
        ["/", "\\"],
        DIRECTORY_SEPARATOR,
        $pluginBackupDir . $filename,
      );
      ...
      if ($this->filesystem->exists($sourceFile)) {
        if (is_dir($sourceFile)) {
          $this->log(
            self::LOG_INFO_PREFIX,
            "It's a directory. Start to mirror it to '$targetFile'.",
          );
        ...
        if (is_file($sourceFile)) {
          $this->log(
            self::LOG_INFO_PREFIX,
            "It's a file. Start to copy it to '$targetFile'.",
          );
        ...
        $this->log(
          self::LOG_WARN_PREFIX,
          "Path '$sourceFile' is not existing or not accessable.",
        );
      }
    }
    ...
    if ($this->configuration->isS3Enabled() && $this->s3Storage->isEnabled()) {
      try {
        $this->log(self::LOG_INFO_PREFIX, "Uploading backup to S3.");
        $this->s3Storage->uploadFile($backupZipName, $backupName . ".zip");
        $this->log(
          self::LOG_INFO_PREFIX,
    ...
    $this->log(
      self::LOG_INFO_PREFIX,
      "--- F I N I S H E D   C R E A T I N G   B A C K U P ---",
    );
  ...
  public function getFilesInDirRecursively(
    string $dir,
    array &$resultFileList = [],
  ): array {
    $files = scandir($dir);
  ...
  public function zipData(string $source, string $destination): bool
  {
    $this->log(
      self::LOG_INFO_PREFIX,
      "Start zipping '$source' to '$destination'.",
    );
  ...
  public function getExistingBackups(): array
  {
    ...
    if ($this->configuration->isS3Enabled() && $this->s3Storage->isEnabled()) {
      try {
        $s3Backups = $this->s3Storage->listBackups();
    ...
    // Sort all backups by timestamp descending
  ...
  public function deleteOldBackups(): array
  {
    ...
    if ($backupAmountMax > 0 && $numBackupsExisting > $backupAmountMax) {
      $this->log(
        self::LOG_INFO_PREFIX,
        "Delete old backups. Max. amount to keep: $backupAmountMax; Existing: $numBackupsExisting",
      );
      ...
      $backupsToDeleteArr = array_slice(
        $existingBackupsArr,
        0,
        $amountToDelete,
      );
      ...
      foreach ($backupsToDeleteArr as $backupToDelete) {
        $location = $backupToDelete["location"] ?? "local";

        if (
          $location === "s3" &&
          $this->configuration->isS3Enabled() &&
          $this->s3Storage->isEnabled()
        ) {
          // Delete from S3
          try {
            $this->s3Storage->deleteFile($backupToDelete["name"]);
            $this->log(
              self::LOG_INFO_PREFIX,
              "Deleted S3 backup '" . $backupToDelete["name"] . "'",
            );
          } catch (\Exception $e) {
            $this->log(
              self::LOG_ERROR_PREFIX,
          ...
          if ($this->filesystem->exists($backupFullPath)) {
            ...
            $this->log(
              self::LOG_INFO_PREFIX,
              "Deleted local backup '$backupFullPath'",
            );
          }
        }
      }
