...
final class EasyBackupController extends AbstractController
{
  ...
  public function restoreAction(Request $request)
  {
    ...
    if (preg_match(self::REGEX_BACKUP_ZIP_NAME, $backupName)) {
      ...
      $zipNameAbsolute = str_replace(
        ["/", "\\"],
        DIRECTORY_SEPARATOR,
        $this->getBackupDirectory() . $backupName,
      );
      $backupName = basename($zipNameAbsolute, ".zip"); // e.g. 2020-11-02_174452
      $restoreDir = str_replace(
        ["/", "\\"],
        DIRECTORY_SEPARATOR,
  ...
  public function prepareRecoveryAction(Request $request)
  {
    ...
    if (preg_match(self::REGEX_BACKUP_ZIP_NAME, $backupName)) {
      ...
      $fileOverwrites = array_filter(
        str_replace($restoreDir, "", $fileOverwrites),
      );
  ...
  private function getFilesInDirRecursively(
    string $dir,
  ...
  private function restoreDirsAndFiles(string $restoreDir): void
  {
    ...
    $this->log(
      ...
      "Blacklist of files not to move: " . implode(", ", $blacklist),
    );
    ...
    foreach ($filePathsToRestore as $filenameAbs) {
      ...
      $filenameAbsNew = str_replace(
        $restoreDir,
        $this->kimaiRootPath,
        $filenameAbs,
      );
      $filepermsSource = substr(sprintf("%o", fileperms($filenameAbs)), -4);
      $filepermsDestination = substr(
        sprintf("%o", fileperms($filenameAbsNew)),
        -4,
      );
      $this->log(
        self::LOG_INFO_PREFIX,
        "Copying '$filenameAbs' (Permissions: $filepermsSource) to '$filenameAbsNew' (Permissions: $filepermsDestination).",
      );
      ...
      if (!is_writable($filenameAbsNew)) {
        ...
        $this->log(
          self::LOG_INFO_PREFIX,
          "Trying to change the file permissions of '$filenameAbsNew' to '$newFilePermissions' automatically.",
        );
        if (chmod($filenameAbsNew, $newFilePermissions)) {
          if (rename($filenameAbs, $filenameAbsNew)) {
            $this->log(
              self::LOG_INFO_PREFIX,
              "Successfully copied '$filenameAbsNew' after file permissions were changed.",
            );
          } else {
            $this->log(
              self::LOG_ERROR_PREFIX,
              "Unable to copy to '$filenameAbsNew'. Please check the file permissions and try it again.",
            );
          }
        } else {
          $this->log(
            self::LOG_ERROR_PREFIX,
            "Failed to change permissions of '$filenameAbsNew' to '$newFilePermissions'.",
          );
        }
      }
    }
  ...
  private function unzip(string $source, string $destination): bool
  {
    $this->log(
      self::LOG_INFO_PREFIX,
      "Start unzipping '$source' to '$destination'.",
    );

    if (\extension_loaded("zip") === true) {
      $zip = new ZipArchive();

      if (file_exists($source) === true && $zip->open($source) === true) {
  ...
  private function checkStatus(): array
  {
    ...
    if (file_exists($dotGitPath)) {
      ...
      $this->log(
        self::LOG_INFO_PREFIX,
        "No git repository recognized. Expected path: " . $dotGitPath,
      );
    }
  ...
  private function restoreMySQLDump(string $restoreDir): void
  {
    ...
    if (\in_array($scheme, ["mysql", "mysqli"])) {
      ...
      $mysqlCmd = str_replace(
        "{sql_file}",
        $restoreDir . self::SQL_DUMP_FILENAME,
        $mysqlCmd,
      );
